...
type TaskQuestion = { type: 'task'; task: typeof TASK_TYPES[number]; priority: typeof PRIORITIES[number]; };
...
const COMBINATIONS: Question[] = [
  ...TASK_TYPES.flatMap((task) =>
    PRIORITIES.map((priority) => ({ type: 'task', task, priority } as TaskQuestion))
  ),
  { type: 'preferenceTime' },
  { type: 'preferredDays' },
  { type: 'focusHours' },
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case 'SET_RANGE':
      return {
        ...state,
        answers: { ...state.answers, [action.key]: { start: action.start, end: action.end } },
        error: '',
      };
...
const SmartSetupPage: React.FC = () => {
  ...
  const { currentIndex, startTime, endTime, answers, preferenceTime, preferredDays, preferredDaysByTask, error, submitted } = state;
    ...
    if (submitted) {
      ...
      fetch("http://localhost:5000/api/preferences", {
        ...
        .catch((err) => {
          ...
          dispatch({ type: 'SET_ERROR', error: err.message || 'Failed to save preferences' });
        });
    }
  }, [submitted, answers, preferenceTime, preferredDays, preferredDaysByTask, router]);

  const current = useMemo(() => COMBINATIONS[currentIndex], [currentIndex]);
  const key = useMemo(() => {
    switch (current.type) {
      case 'task': return `${current.task}_${current.priority}`;
      case 'taskDays': return `PreferredDays_${current.task}`;
  ...
  useEffect(() => {
    ...
    dispatch({ type: 'SET_START_END', start: existing?.start || '', end: existing?.end || '' });
  }, [key]);

  const handleNext = useCallback(() => {
    if (current.type === 'task' || current.type === 'focusHours' || current.type === 'avoidTimes') {
      if (!startTime || !endTime) {
        dispatch({ type: 'SET_ERROR', error: 'Please select both start and end times.' });
        return;
      }
      if (startTime >= endTime) {
        dispatch({ type: 'SET_ERROR', error: 'End time must be later than start time.' });
        return;
      }
  ...
  const handleBack = useCallback(() => {
    if (currentIndex > 0) dispatch({ type: 'SET_INDEX', index: currentIndex - 1 });
  }, [currentIndex]);
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      ...
      {!submitted ? (
        <>
          {(current.type === 'task' || current.type === 'focusHours' || current.type === 'avoidTimes') && (
            <TimeRangeQuestion
              label={
                current.type === 'task'
                  ? `Question ${currentIndex + 1}: When do you usually perform a ${current.task} task with ${current.priority} priority?`
                  : current.type === 'focusHours'
                  ? `Question ${currentIndex + 1}: During which hours do you feel most focused?`
                  : `Question ${currentIndex + 1}: Are there any times you'd like to avoid for tasks?`
              }
              start={startTime}
              end={endTime}
              onStart={val => dispatch({ type: 'SET_START_END', start: val, end: endTime })}
              onEnd={val => dispatch({ type: 'SET_START_END', start: startTime, end: val })}
            />
          )}
          {current.type === 'preferenceTime' && (
            <div className="form-control">
              <label className="label">Question {currentIndex + 1}: What time of day do you prefer for tasks?</label>
              <select
                aria-label="Preference Time"
                className="select select-bordered"
                value={preferenceTime}
                onChange={e => dispatch({ type: 'SET_PREFERENCE_TIME', value: e.target.value })}
              >
                <option value="">Select preference</option>
                {['Morning', 'Afternoon', 'Evening', 'No preference'].map(opt => (
          ...
          {current.type === 'preferredDays' && (
            <CheckboxGroup
              label={`Question ${currentIndex + 1}: Which days are preferred for scheduling tasks?`}
          ...
          {current.type === 'taskDays' && (
            <CheckboxGroup
              label={`Question ${currentIndex + 1}: Which days are preferred for ${current.task} tasks?`}
              options={DAYS_OF_WEEK}
              selected={preferredDaysByTask[current.task] || []}
              onToggle={day => dispatch({ type: 'TOGGLE_TASK_DAY', task: current.task, day })}
            />
          )}
          {error && <p className="text-red-500 font-medium">{error}</p>}
          <div className="flex justify-between mt-4">
            {currentIndex > 0 && <button onClick={handleBack} className="btn btn-outline btn-info">Back</button>}
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-green-600">All answers have been submitted!</h2>
